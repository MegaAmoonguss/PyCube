...
from js2py.pyjs import * #@UnusedWildImport
# setting scope
var = Scope( JS_BUILTINS )
set_global_object(var)

# Code follows:
var.registers(['flat2posit', 'appendmoves', 'colors', 'size', 'cubeorient', 'mult', 'colorList', 'scramblestring', 'seq', 'doslice', 'posit', 'colorPerm', 'scramble', 'setForm', 'numcub', 'seqlen', 'imagestring', 'colorString', 'help'])
@Js
def PyJsHoisted_appendmoves_(sq, axsl, tl, la, this, arguments, var=var):
    ...
    var.registers(['la', 'tl', 'sl', 'sa', 'axsl', 'm', 'q', 'sq'])
    #for JS loop
    var.put('sl', Js(0.0))
...
def PyJsHoisted_scramble_(this, arguments, var=var):
    ...
    var.registers(['axam', 'la', 'tl', 'sl', 'moved', 'axsl', 'ax', 'i', 'q'])
...
def PyJsHoisted_imagestring_(nr, this, arguments, var=var):
    ...
    var.registers(['nr', 'f', 'd', 'ori', 's', 'stickerheight', 'imageheight', 'i', 'q', 'c'])
    var.put('s', Js(''))
    var.put('d', Js(0.0))
    #for JS loop
    var.put('i', Js(0.0))
    ...
    var.put('s', ((((Js("<div style='width:")+((var.get('stickerheight')*var.get('size'))*Js(4.0)))+Js('px; height:'))+((var.get('stickerheight')*var.get('size'))*Js(3.0)))+Js("px;'>")))
    #for JS loop
    var.put('i', Js(0.0))
    while (var.get('i')<(Js(3.0)*var.get('size'))):
        try:
            var.put('s', ((((Js("<div style='float: left; display: block; height: ")+var.get('stickerheight'))+Js('px; width: '))+((var.get('stickerheight')*var.get('size'))*Js(4.0)))+Js("px; line-height: 0px;'>")), '+')
            #for JS loop
            var.put('f', Js(0.0))
            while (var.get('f')<(Js(4.0)*var.get('size'))):
                try:
                    if (var.get('flat2posit').get(var.get('d'))<Js(0.0)):
                        var.put('s', ((((Js("<div style='overflow: hidden; display: block; float: left; height: ")+var.get('stickerheight'))+Js('px; width: '))+var.get('stickerheight'))+Js("px;'></div>")), '+')
                    else:
                        var.put('c', var.get('colorPerm').get(var.get('ori')).get(var.get('posit').get(var.get('flat2posit').get(var.get('d')))))
                        var.put('s', ((((((Js("<div style='overflow: hidden; display: block; float: left; border: 1px solid #000; height: ")+((var.get('stickerheight')*Js(1.0))-Js(2.0)))+Js('px; width: '))+((var.get('stickerheight')*Js(1.0))-Js(2.0)))+Js("px;'><img src='scrbg/"))+var.get('colorList').get((var.get('colors').get(var.get('c'))+Js(1.0))))+Js("' /></div>")), '+')
                    (var.put('d',Js(var.get('d').to_number())+Js(1))-Js(1))
                finally:
                        (var.put('f',Js(var.get('f').to_number())+Js(1))-Js(1))
            var.put('s', Js('</div>'), '+')
...
def PyJsHoisted_doslice_(f, d, q, this, arguments, var=var):
    ...
    var.registers(['f', 'd', 's2', 'f1', 'f4', 'q', 'f3', 'f2', 'i', 'j', 'k', 'c'])
    pass
